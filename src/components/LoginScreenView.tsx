...
export const LoginScreen = ({ onLogin }: { onLogin: (phone: string, uid: string) => void }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white text-slate-900 p-6 relative overflow-hidden">
      <div id="recaptcha-container" className="z-10"></div>
      ...
      >
        ...
        <div className="w-full">
          {isConfigMissing && (
            <div className="bg-red-50 border border-red-200 text-red-600 p-4 rounded-xl mb-6 text-sm flex flex-col items-center text-center">
          ...
          {step === 'phone' ? (
            <form onSubmit={handleSendOtp} className="space-y-6 w-full">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl py-4 pl-14 pr-4 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary text-lg tracking-wider"
              ...
              <button
                ...
                className="w-full bg-primary hover:bg-orange-700 text-white font-bold py-4 rounded-xl transition flex items-center justify-center shadow-lg shadow-orange-600/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <div className="mt-4 pt-4 border-t border-slate-200 space-y-3">
                <button
                  type="button"
                  onClick={() => onLogin('8888888888', 'demo-uid-123')}
                  className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 rounded-xl transition flex items-center justify-center border border-slate-300"
                >
                  Bypass Login (User Demo)
                </button>
                <button
                  type="button"
                  onClick={() => onLogin('9999999999', 'admin-uid-456')}
                  className="w-full bg-orange-50 hover:bg-orange-100 text-orange-600 font-bold py-3 rounded-xl transition flex items-center justify-center border border-orange-200"
                >
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-6 w-full">
              <div>
                ...
                <p className="text-xs text-slate-500 mb-4">Sent to +91 {phone} <button type="button" onClick={() => setStep('phone')} className="text-orange-600 hover:underline ml-1">Edit</button></p>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl py-4 pl-10 pr-4 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500 text-center text-2xl tracking-[0.5em] font-mono"
                    placeholder="------"
                    required
                  />
                </div>
